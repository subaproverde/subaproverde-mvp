...
export default function MlConnectCard({ sellerId }: { sellerId: string }) {
  ...
  function startConnect() {
    // ✅ se você está numa página que já tem sellerId, conecta por sellerId (fluxo antigo)
    if (!sellerId) return;
    window.location.href = `/api/ml/connect?sellerId=${encodeURIComponent(sellerId)}`;
  }

  function startConnectByUser() {
    // ✅ para casos onde você quer conectar “do zero” por userId
    if (!user?.id) return;
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
      ...
      <div className="mt-4 grid gap-2">
        {/* Quando tem sellerId (página do seller específico) */}
        {!!sellerId && !connected && (
          <button
        ...
        {/* Fallback: conectar por userId (caso esteja sem sellerId ainda) */}
        {!sellerId && !connected && (
          <button
            onClick={startConnectByUser}
            className="w-full rounded-xl px-4 py-2 border border-white/10 bg-white/5 text-white/90 hover:bg-white/10"
          >
            Conectar Mercado Livre
          </button>
