...
const statusStyle: Record<string, string> = {
  ...
  paid: "bg-emerald-300/10 text-spv-accent-text", overdue: "bg-rose-300/10 text-rose-100", cancelled: "bg-spv-surface text-spv-muted",
};
...
export default function FinanceClient() {
  ...
  return <div className="min-w-0">
    ...
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-spv-accent-text">Financeiro integrado ao atendimento</div>
        <h1 className="mt-1 text-3xl font-semibold tracking-tight text-spv-ink">Contas a receber</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-spv-muted">Pedido confirmado gera cobrança. Pagamento parcial reduz o saldo e comprovante conciliado baixa a conta — com divergências sempre visíveis.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => setCreating(true)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-emerald-400 px-3.5 text-sm font-semibold text-spv-on-accent hover:bg-emerald-300"><Plus className="h-4 w-4" /> Nova conta</button>
        <button type="button" onClick={() => void load()} className="inline-flex h-10 items-center gap-2 rounded-xl border border-spv-line bg-spv-surface px-3 text-sm text-spv-muted hover:bg-spv-raised"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Atualizar</button>
      </div>
    </header>
    ...
    {pendingReceipts.length ? <section className="mt-5 rounded-xl border border-amber-300/15 bg-amber-300/[0.035] p-4 sm:p-5">
      <div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-sm font-semibold text-spv-ink"><FileCheck2 className="h-4 w-4 text-amber-200" /> Comprovantes para conciliar</h2><p className="mt-1 text-xs text-spv-muted">A baixa só acontece quando o valor bate ou você confirma uma parcial.</p></div><span className="rounded-full bg-amber-300/10 px-2.5 py-1 text-xs font-semibold text-amber-100">{pendingReceipts.length}</span></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{pendingReceipts.map((receipt) => <article key={receipt.id} className="rounded-xl border border-spv-line bg-spv-page p-4">
        <div className="flex items-start justify-between gap-3"><div><div className="font-semibold text-spv-ink">{contactName(receipt.contact)}</div><div className="mt-1 text-xs text-spv-muted">Recebido em {shortDate(receipt.createdAt)}</div></div><div className="text-right"><div className="text-lg font-semibold text-amber-100">{receipt.extractedAmount || receipt.claimedAmount ? money.format(receipt.extractedAmount || receipt.claimedAmount) : "Valor não lido"}</div><div className="text-[10px] text-spv-muted">confiança {Math.round(receipt.confidence * 100)}%</div></div></div>
    ...
    <section className="mt-5 overflow-hidden rounded-xl border border-spv-line bg-spv-surface">
      <div className="border-b border-spv-line p-4 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2">{(["open", "overdue", "paid", "all"] as const).map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={`h-9 rounded-xl px-3 text-xs font-semibold ${filter === item ? "bg-emerald-300/12 text-spv-accent-text" : "bg-spv-surface text-spv-muted hover:text-spv-ink"}`}>{item === "open" ? "Em aberto" : item === "overdue" ? "Vencidas" : item === "paid" ? "Pagas" : "Todas"}</button>)}</div><label className="relative block lg:w-80"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-spv-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar cliente ou serviço" className="h-10 w-full rounded-xl border border-spv-line bg-spv-page pl-9 pr-3 text-sm text-spv-ink outline-none placeholder:text-spv-muted focus:border-emerald-300/25" /></label></div>
      </div>
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[880px] text-left"><thead><tr className="border-b border-spv-line text-[10px] uppercase tracking-[0.1em] text-spv-muted"><th className="px-5 py-3 font-semibold">Cliente / serviço</th><th className="px-4 py-3 font-semibold">Vencimento</th><th className="px-4 py-3 text-right font-semibold">Total</th><th className="px-4 py-3 text-right font-semibold">Recebido</th><th className="px-4 py-3 text-right font-semibold">Saldo</th><th className="px-4 py-3 font-semibold">Situação</th><th className="px-5 py-3 text-right font-semibold">Ação</th></tr></thead><tbody>{filtered.map((row) => <tr key={row.id} className="border-b border-spv-line last:border-0 hover:bg-spv-raised"><td className="px-5 py-4"><div className="font-medium text-spv-ink">{contactName(row.contact)}</div><div className="mt-1 max-w-sm truncate text-xs text-spv-muted">{row.description || "Serviço Suba Pro Verde"}{row.source === "order" ? " · gerada pelo pedido" : row.source === "direct_payment" ? " · pagamento direto" : ""}</div></td><td className="px-4 py-4 text-sm text-spv-muted">{shortDate(row.dueDate)}</td><td className="px-4 py-4 text-right text-sm text-spv-muted">{money.format(row.amount)}</td><td className="px-4 py-4 text-right text-sm text-spv-accent-text">{money.format(row.paidAmount)}</td><td className="px-4 py-4 text-right font-semibold text-spv-ink">{money.format(row.openAmount)}</td><td className="px-4 py-4"><Status status={row.status} /></td><td className="px-5 py-4 text-right">{!["paid", "cancelled"].includes(row.status) ? <button type="button" onClick={() => setSettling(row)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-emerald-400 px-3 text-xs font-semibold text-spv-on-accent"><Check className="h-3.5 w-3.5" /> Dar baixa</button> : <span className="text-xs text-spv-muted">Concluída</span>}</td></tr>)}</tbody></table></div>
      <div className="divide-y divide-white/[0.07] md:hidden">{filtered.map((row) => <article key={row.id} className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-semibold text-spv-ink">{contactName(row.contact)}</div><div className="mt-1 line-clamp-2 text-xs leading-5 text-spv-muted">{row.description || "Serviço Suba Pro Verde"}</div></div><Status status={row.status} /></div><div className="mt-4 grid grid-cols-3 gap-2"><SmallValue label="Total" value={money.format(row.amount)} /><SmallValue label="Recebido" value={money.format(row.paidAmount)} /><SmallValue label="Saldo" value={money.format(row.openAmount)} strong /></div><div className="mt-3 flex items-center justify-between text-xs text-spv-muted"><span>{shortDate(row.dueDate)}</span>{!["paid", "cancelled"].includes(row.status) ? <button type="button" onClick={() => setSettling(row)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-emerald-400 px-3 font-semibold text-spv-on-accent"><Check className="h-3.5 w-3.5" /> Dar baixa</button> : null}</div></article>)}</div>
      {!loading && !filtered.length ? <div className="px-6 py-16 text-center"><WalletCards className="mx-auto h-8 w-8 text-spv-muted" /><div className="mt-3 text-sm font-medium text-spv-muted">Nenhuma conta neste filtro</div><div className="mt-1 text-xs text-spv-muted">Pedidos confirmados e cobranças manuais aparecerão aqui.</div></div> : null}
    </section>
...
function Metric({ icon: Icon, label, value, detail, tone }: { icon: typeof CircleDollarSign; label: string; value: string; detail: string; tone: string }) {
  const tones: Record<string, string> = { emerald: "text-spv-accent-text bg-emerald-300/[0.08]", rose: "text-rose-200 bg-rose-300/[0.08]", sky: "text-sky-200 bg-sky-300/[0.08]", amber: "text-amber-200 bg-amber-300/[0.08]", violet: "text-violet-200 bg-violet-300/[0.08]" };
  return <article className="rounded-xl border border-spv-line bg-spv-surface p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-spv-muted">{label}</span><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${tones[tone]}`}><Icon className="h-4 w-4" /></span></div><div className="mt-3 truncate text-xl font-semibold text-spv-ink">{value}</div><div className="mt-1 text-xs text-spv-muted">{detail}</div></article>;
}
...
function SmallValue({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) { return <div className="rounded-xl bg-spv-page p-2"><div className="text-[9px] uppercase tracking-wide text-spv-muted">{label}</div><div className={`mt-1 truncate text-xs ${strong ? "font-semibold text-spv-ink" : "text-spv-muted"}`}>{value}</div></div>; }

function DialogShell({ title, subtitle, children, onClose }: { title: string; subtitle: string; children: React.ReactNode; onClose: () => void }) { return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/70 p-0 backdrop-blur-none-sm sm:items-center sm:p-4"><div className="max-h-[92vh] w-full overflow-y-auto rounded-t-[26px] border border-spv-line bg-spv-page p-5 shadow-2xl sm:max-w-lg sm:rounded-xl"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-spv-ink">{title}</h2><p className="mt-1 text-sm text-spv-muted">{subtitle}</p></div><button type="button" onClick={onClose} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-spv-surface text-spv-muted"><X className="h-4 w-4" /></button></div>{children}</div></div>; }
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-1.5 block text-xs font-semibold text-spv-muted">{label}</span>{children}</label>; }
function SubmitButton({ saving, label }: { saving: boolean; label: string }) { return <button type="submit" disabled={saving} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 text-sm font-semibold text-spv-on-accent disabled:opacity-50">{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{label}</button>; }
