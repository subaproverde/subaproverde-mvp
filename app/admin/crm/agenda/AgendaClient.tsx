...
import { BellRing, CalendarCheck2, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, Columns3, Expand, LayoutGrid, List, Minimize2, Plus, RefreshCw, UserRound, X } from "lucide-react";
...
export default function AgendaClient() {
  ...
  async function createTask(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await authFetch("/api/admin/crm/agenda", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(form.entries())) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "Falha ao criar compromisso."); setShowForm(false); await load();
    } catch (failure) { setError(failure instanceof Error ? failure.message : "Falha ao criar compromisso."); }
    finally { setSaving(false); }
  }
  async function markDone(taskId: string) {
    const response = await authFetch("/api/admin/crm/agenda", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ taskId, status: "done" }) });
  ...
  return <div className="crm-agenda">
    ...
    {showForm ? <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={() => setShowForm(false)}><form onSubmit={createTask} onMouseDown={(event) => event.stopPropagation()} className="agenda-dialog max-h-[94vh] w-full overflow-y-auto rounded-t-[26px] p-5 shadow-2xl sm:max-w-xl sm:rounded-[26px] sm:p-6"><div className="flex items-start justify-between gap-4"><div><div className="agenda-title flex items-center gap-2 font-semibold"><CalendarCheck2 className="h-4 w-4 text-emerald-400" /> Novo compromisso</div><p className="agenda-muted mt-1 text-sm">O calendário avisará visualmente quando o horário estiver próximo.</p></div><button type="button" onClick={() => setShowForm(false)} className="agenda-control flex h-9 w-9 shrink-0 items-center justify-center"><X className="h-4 w-4" /></button></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="sm:col-span-2"><FieldLabel>Título</FieldLabel><input name="title" required placeholder="Ex.: Retornar orçamento" className="agenda-input" /></label><label><FieldLabel>Data e horário</FieldLabel><input name="dueAt" type="datetime-local" required defaultValue={localInputValue(new Date(Date.now() + 3_600_000))} className="agenda-input" /></label><label><FieldLabel>Tipo</FieldLabel><select name="taskType" className="agenda-input"><option value="follow_up">Follow-up</option><option value="appointment">Atendimento</option><option value="anydesk">Acesso remoto</option><option value="internal">Tarefa interna</option></select></label><label><FieldLabel>Cliente</FieldLabel><select name="contactId" defaultValue={initialContactId} className="agenda-input"><option value="">Sem cliente vinculado</option>{contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.name || contact.company_name || contact.phone}</option>)}</select></label><label><FieldLabel>Prioridade</FieldLabel><select name="priority" className="agenda-input"><option value="medium">Normal</option><option value="high">Alta</option><option value="urgent">Urgente</option><option value="low">Baixa</option></select></label><label className="sm:col-span-2"><FieldLabel>Observação</FieldLabel><textarea name="description" rows={4} placeholder="O que precisa acontecer neste compromisso?" className="agenda-input h-auto py-3" /></label></div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setShowForm(false)} className="agenda-control h-10 px-4 text-sm">Cancelar</button><button disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-xl bg-emerald-400 px-4 text-sm font-semibold text-[#052216] hover:bg-emerald-300 disabled:opacity-50">{saving ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Salvar</button></div></form></div> : null}
    {selectedTask ? <TaskDetails task={selectedTask} now={now} onClose={() => setSelectedTask(null)} onDone={markDone} /> : null}
  </div>;
}
