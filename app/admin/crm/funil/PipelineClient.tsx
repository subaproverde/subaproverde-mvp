...
export default function PipelineClient() {
  ...
  return <div>
    <CrmSectionNav />
    <header className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300/75">Pipeline comercial</div><h1 className="mt-1 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Funil de vendas</h1><p className="mt-2 text-sm text-white/43">Deslize pelas etapas e mova cada lead pelo seletor do cartão.</p></div><div className="grid w-full grid-cols-3 gap-2 lg:w-auto"><Summary label="Oportunidades" value={String(visible.length)} /><Summary label="Qualificados" value={String(qualified)} /><Summary label="Potencial" value={money.format(totalValue)} /></div></header>
    <div className="mt-5 flex min-w-0 gap-2 sm:gap-3"><label className="relative min-w-0 max-w-lg flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-white/28" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar no funil" className="h-10 w-full rounded-xl border border-white/[0.08] bg-white/[0.03] pl-10 pr-3 text-sm text-white outline-none placeholder:text-white/24 focus:border-emerald-300/30" /></label><button type="button" onClick={() => void load()} className="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border border-white/[0.08] px-3 text-sm text-white/58 hover:bg-white/[0.05]" aria-label="Atualizar funil"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /><span className="hidden sm:inline">Atualizar</span></button></div>
